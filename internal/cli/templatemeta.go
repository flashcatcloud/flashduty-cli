...
// CLI owns it directly. Platform-side additions require a CLI release.
...
// presetTemplateResult is the CLI output for `template get-preset`.
type presetTemplateResult struct {
	Channel      string `json:"channel"`
	FieldName    string `json:"field_name"`
	TemplateCode string `json:"template_code"`
}

// validateTemplateResult is the CLI output for `template validate`.
...
// templateVariable describes one template variable for `template variables`.
...
// templateFunction describes one template function for `template functions`.
