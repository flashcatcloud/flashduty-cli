...
Response fields ('data' envelope is unwrapped — rows are nested under items[]; pipe 'jq '.items[]'', NOT '.data.items[]'):
  ...
  - items (array<object>) (required) — Preset severity rule history snapshots of the current page.
    - rules (array<object>) (required) — Full rule set captured immediately before the mutation that produced this snapshot. Empty for the very first snapshot.
      ...
      - deleted_at (string) (required) — Unix timestamp in milliseconds the rule was soft-deleted; '0' means not deleted. Always '0' in practice, since deleted rules are excluded before a snapshot is taken. CLI '--json' renders this as an RFC3339 string in the process's local timezone (NOT UTC, and NOT the wire integer); an unset value renders as null.
    ...
    - updated_at (string) (required) — Unix timestamp in milliseconds when the snapshot was written. CLI '--json' renders this as an RFC3339 string in the process's local timezone (NOT UTC, and NOT the wire integer); an unset value renders as null.
