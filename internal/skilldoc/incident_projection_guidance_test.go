...
func TestIncidentCardAvoidsUnboundedStructuredHotFlows(t *testing.T) {
	...
	for description, command := range map[string]string{
		"triage list":    "incident list --severity Critical --progress Triggered --since 4h --fields incident_id,num,title,incident_severity,progress,start_time,channel_id,detail_url --output-format toon",
		"triage detail":  "incident detail <incident-id> --fields incident_id,num,title,incident_severity,progress,ai_summary,root_cause,resolution,alert_cnt,start_time,channel_id,detail_url --output-format toon",
		"summary detail": `incident detail   "$ID" --fields incident_id,num,title,incident_severity,progress,ai_summary,root_cause,resolution,alert_cnt,start_time,channel_id,detail_url --output-format toon`,
	} {
